...
{ 
	...
	public class StoryScript19 : MonoBehaviour
	{
		[SerializeField] 
		...
		{
			if (other.tag == "Player" && GlobalsScript.StoryFlagsArray[18] == false)
			{
				PlayerTalk.text = GlobalStringText.PlayerTalkStrings[18];
				ParasiteTalk.text = GlobalStringText.ParasiteTalkStrings[18];
				//	Player_RigidBody.constraints = RigidbodyConstraints2D.FreezePosition;//Freeze till dialog is done	
				GlobalsScript.StoryFlagsArray[18] = true; //Changes the first story flag to false.
		...
		private void ONDONE()
		{
			PlayerTalk.text = "You won't get away with this, I'll tell the police, the government!";
			ParasiteTalk.text = "Who do you think runs them?"; 
