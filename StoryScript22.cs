...
{ 
	...
	public class StoryScript22 : MonoBehaviour
	{
		...
		{
			if (other.tag == "Player" && GlobalsScript.StoryFlagsArray[21] == false)
			{
				PlayerTalk.text = GlobalStringText.PlayerTalkStrings[21];
				ParasiteTalk.text = GlobalStringText.ParasiteTalkStrings[21];
				//		Player_RigidBody.constraints = RigidbodyConstraints2D.FreezePosition;//Freeze till dialog is done	
				GlobalsScript.StoryFlagsArray[21] = true; //Changes the first story flag to false.
