...
public class RandomEncounterScriptZone2 : MonoBehaviour //This is for the sewer maze ZONE 1.
{
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		if (other.tag == "Player")
		{
			...
			if(RandomEncounterRoll == 98)
			{
				...
				GlobalsScript.RandomEncounterCategory = 3;//Evil Doctor
			...
			else if(RandomEncounterRoll == 3)
			{
				...
				GlobalsScript.RandomEncounterCategory = 4; //Clean Up Crew 1
				GlobalsScript.lastposition.x = PlayerPosition.position.x;
			...
			else if(RandomEncounterRoll == 30)
			{
				...
				GlobalsScript.RandomEncounterCategory = 2; //Rat
			...
			else if(RandomEncounterRoll == 77)
			{
				...
				GlobalsScript.RandomEncounterCategory = 1; //Evil Nurse
				GlobalsScript.lastposition.x = PlayerPosition.position.x;
